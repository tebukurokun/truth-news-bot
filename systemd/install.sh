...
do_check() {
  ...
  if [[ $rc -eq 0 ]]; then
    ...
    echo "" >&2
    echo "差分あり（- が配置済み / + がリポジトリ）。次のどちらかです:" >&2
    echo "  1. リポジトリ側が新しい（git pull 後の通常のデプロイ）" >&2
    echo "     → sudo $0 install で反映する" >&2
    echo "  2. VPS 上で直接編集された（応急処置など）" >&2
    echo "     → その変更をリポジトリに取り込んで commit してから install する" >&2
  fi
  return $rc
}
